...
//controllers are basically the functions for each of the api calls
...
export const UserRegister = async (req, res, next) => { 
  try {
    ...
    const createdUser = await user.save(); // the req gets saved in the mongodb database
    const token = jwt.sign({ id: createdUser._id }, process.env.JWT, { // to generate the jwt token
...
export const addWorkout = async (req, res, next) => {
  try {
    ...
    // Calculate calories burnt for each workout and create workouts
    const createdWorkouts = await Promise.all(parsedWorkouts.map(async (workout) => {
      workout.caloriesBurned = parseFloat(calculateCaloriesBurnt(workout));
      const createdWorkout = await Workout.create({ ...workout, user: userId });
      return createdWorkout.toObject(); // Convert to plain JavaScript object
    }));

    return res.status(201).json({
      message: "Workouts added successfully",
      workouts: createdWorkouts, // Return the created workouts
    });
  } catch (err) {
    next(err);
  }
...
export const deleteWorkout = async (req, res, next) => {
  try {
    ...
    // Find the workout by ID and verify it belongs to the current user
